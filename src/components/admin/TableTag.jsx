...
const TableTag = () => {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        const response = await fetch("http://localhost:9999/api/tag/get-all");
        if (response.ok) {
  ...
  const handleCreateType = async () => {
    try {
      const response = await fetch("http://localhost:9999/api/tag/create", {
        method: "POST",
  ...
  const handleDeleteType = async (id) => {
    try {
      const response = await fetch(`http://localhost:9999/api/tag/delete/${id}`, {
        method: "DELETE",
      });
