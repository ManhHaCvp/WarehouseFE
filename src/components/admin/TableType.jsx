...
const TableType = () => {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        const response = await fetch("http://localhost:9999/api/type/get-all");
        if (response.ok) {
  ...
  const handleCreateType = async () => {
    try {
      const response = await fetch("http://localhost:9999/api/type/create", {
        method: "POST",
  ...
  const handleDeleteType = async (id) => {
    try {
      const response = await fetch(`http://localhost:9999/api/type/delete/${id}`, {
        method: "DELETE",
      });
